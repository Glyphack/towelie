...
%o`, message, error2, detail);
  ...
  var CommentStorage = class _CommentStorage {
    ...
    static {
      this.KEY = "towelie-comments";
    }
    load() {
      const stored = localStorage.getItem(_CommentStorage.KEY);
      if (stored) {
        try {
          this.comments = JSON.parse(stored);
        } catch {
          this.comments = [];
          localStorage.removeItem(_CommentStorage.KEY);
        }
      }
    }
    add(selection, text, branch) {
    ...
    save() {
      localStorage.setItem(_CommentStorage.KEY, JSON.stringify(this.comments));
    }
  };
  function populateBranchSelects(branchSelect, baseBranchSelect, data) {
  ...
  function populateCommitSelect(commitSelect, data) {
  ...
  function renderTreeLevel(parent, tree, hrefMap, depth) {
  ...
  function renderCommentIndicators(outputEl, comments, onDelete) {
    ...
    outputEl.querySelectorAll(".towelie-comment-popup").forEach((popup) => popup.remove());
  ...
  var ReviewController = class extends Controller {
    constructor() {
      super(...arguments);
    ...
    async finishReview(e) {
      ...
      if (branchComments.length === 0) {
        flashButton(btn, "No comments to copy", 2e3);
        return;
      }
